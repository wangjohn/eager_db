...
class SqlParserTest < EagerDB::Test
  def test_single_bind_value_statement
    ...
    assert_equal "SELECT * FROM some_table WHERE name = ?", parsed_result.non_binded_sql
  end
  ...
  def test_multiple_bind_values_statement
    ...
    assert_equal "SELECT * FROM some_table WHERE name = ? AND created_at > ?", parsed_result.non_binded_sql
  end
  ...
  def test_correctly_find_multiple_tables
    ...
    parsed_result = EagerDB::SqlStatement.new(sql)
    assert_equal ["'hello'"], parsed_result.bind_values
    assert_equal "SELECT * FROM some_table AS st, another_table AS at WHERE st.name = ?", parsed_result.non_binded_sql
  end
  ...
  def test_correctly_find_group_by_values
    ...
    assert_equal "SELECT COUNT(*) FROM some_table WHERE id > ? GROUP BY name", parsed_result.non_binded_sql
  end
end
